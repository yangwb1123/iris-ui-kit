...
function cloneKanbanColumns(columns: KanbanColumn[]): KanbanColumn[] {
...
class KanbanStoreEngine {
  readonly store: KanbanStore

  constructor(config: KanbanConfig) {
    const store = createStore<KanbanState>({ columns: cloneKanbanColumns(config.columns) })

    const moveCard = (cardId: string, toColumnId: string): void => {
      const { columns } = store.getState()
      // Find which column currently owns the card.
      const location = findKanbanCard(columns, cardId)
      if (!location) return // card not found
      const { columnIndex: fromColIndex, cardIndex } = location
      ...
      const newColumns = cloneKanbanColumns(columns)
...
export function createKanban(config: KanbanConfig): KanbanStore {
  return new KanbanStoreEngine(config).store
}
