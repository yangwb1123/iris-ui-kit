...
describe('useDataSource (solid)', () => {
  it('loads on mount and re-sorts reactively', async () => {
    const { result } = renderHook(() =>
      useDataSource({ fetcher: createClientDataSource(data, columns), pageSize: 10 }),
    )
    await waitFor(() => expect(result.state().rows).toHaveLength(3))
    result.setSort({ key: 'age', direction: 'asc' })
    await waitFor(() => expect(result.state().rows.map((r) => r.age)).toEqual([25, 30, 35]))
  })
  ...
  it('infinite mode appends via loadMore', async () => {
    ...
    await waitFor(() => expect(result.state().rows).toHaveLength(2))
