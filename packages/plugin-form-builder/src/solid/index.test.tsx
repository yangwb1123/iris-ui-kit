...
describe('IrisFormBuilder (solid)', () => {
  ...
  it('array field: add/remove rows and nested sub-fields re-key on remove', async () => {
    ...
    fireEvent.click(add)
    await waitFor(() => expect(arrayEl.querySelectorAll('[data-iris-fb-row]')).toHaveLength(1))
    ...
    fireEvent.input(sku0, { target: { value: 'AAA' } })
    await waitFor(() =>
      expect(
    ...
    fireEvent.click(add)
    await waitFor(() => expect(arrayEl.querySelectorAll('[data-iris-fb-row]')).toHaveLength(2))
    const sku1 = container.querySelector(
      '[data-iris-form-field="items[1].sku"] input',
    ) as HTMLInputElement
    fireEvent.input(sku1, { target: { value: 'BBB' } })
