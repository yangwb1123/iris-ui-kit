 ...
 * the canonical token set in @iris-ui/tokens, and reports:
...
export async function run() {
  ...
  if (unknown.length > 0) {
    console.log(`\n  WARNING: ${unknown.length} token(s) used but NOT defined in @iris-ui/tokens:\n`)
    ...
    console.log(`\n  All ${allTokens.size} tokens are defined in @iris-ui/tokens.`)
  }
