...
describe('@iris-ui/react IrisFormField', () => {
  ...
  it('label htmlFor matches the input id (auto-generated)', () => {
    const { container } = render(
  ...
  it('forwards ref to the wrapping div', () => {
    ...
    render(
      <IrisFormField ref={ref} label="X">
        <IrisInput />
      </IrisFormField>,
    )
    expect(ref.current).toBeInstanceOf(HTMLDivElement)
    expect(ref.current?.getAttribute('data-iris-form-field')).toBe('')
  })

  it('only injects into the FIRST valid element', () => {
    const { container } = render(
      <IrisFormField label="X">
        <IrisInput data-which="first" />
        <IrisInput data-which="second" />
      </IrisFormField>,
    )
