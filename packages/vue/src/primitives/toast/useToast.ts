import { clearToasts, dismissToast, pushToast, type IrisToastInput } from './store'
...
/**
 * Composable for the global toast queue. Backed by a module-level singleton
 * store, so it works **without** a surrounding `<ToastProvider>` — but the
 * page must mount exactly one `<IrisToastViewport>` somewhere for toasts to
 * actually be visible.
 *
 * @example
