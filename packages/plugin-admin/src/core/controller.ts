...
export function validateAdminDraft(
  ...
  for (const column of columns) {
    ...
    if (column.required && empty) {
      errors[field] = replaceParams('required', { field: column.title })
      continue
    }
    if (empty) continue
    if (column.type === 'number') {
      ...
      if (!Number.isFinite(number)) {
        errors[field] = replaceParams('invalidNumber', { field: column.title })
        continue
      }
      if (column.min !== undefined && number < column.min) {
        errors[field] = replaceParams('min', { field: column.title, min: column.min })
      } else if (column.max !== undefined && number > column.max) {
        errors[field] = replaceParams('max', { field: column.title, max: column.max })
      }
    }
    if (column.type === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(value))) {
      errors[field] = replaceParams('invalidEmail', { field: column.title })
    }
    if (column.pattern && !new RegExp(column.pattern).test(String(value))) {
      errors[field] = replaceParams('pattern', { field: column.title })
    }
    if (
      column.type === 'select' &&
      column.options &&
      ...
      errors[field] = replaceParams('invalidOption', { field: column.title })
    }
  }
  return errors
}
