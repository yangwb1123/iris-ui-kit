...
describe('IrisTable tree rows', () => {
  ...
  it('applies local filters before projecting tree rows', () => {
    const { container } = render(IrisTable, {
      props: { columns: treeCols, data: treeData, getSubRows, filters: { name: 'Root B' } },
    })
    expect(visibleNames(container)).toEqual(['Root B'])
  })
  ...
  it('lazyLoad shows a caret, loads children once, and expands the row', async () => {
    const root = { id: 1, name: 'Lazy root' }
    let loadChildren!: (children: Array<Record<string, unknown>>) => void
    const lazyLoad = vi.fn((_row: Record<string, unknown>, load: typeof loadChildren) => {
      loadChildren = load
    })
    const { container } = render(IrisTable, {
      props: { columns: treeCols, data: [root], rowKey: 'id', lazyLoad },
    })

    expect(visibleNames(container)).toEqual(['Lazy root'])
    const toggle = toggleAt(container, 0)!
    expect(toggle).not.toBeNull()
    await fireEvent.click(toggle)
    expect(lazyLoad).toHaveBeenCalledWith(root, expect.any(Function))
    expect(toggle.getAttribute('data-iris-tree-loading')).toBe('')

    loadChildren([{ id: 11, name: 'Loaded child' }])
    await waitFor(() => expect(visibleNames(container)).toEqual(['Lazy root', 'Loaded child']))
    expect(lazyLoad).toHaveBeenCalledTimes(1)
    expect(toggleAt(container, 0)!.getAttribute('aria-expanded')).toBe('true')
  })

  it('treats an initial empty children array as a lazy placeholder and caches loaded empty children', async () => {
    const root = { id: 1, name: 'Lazy empty', children: [] as Array<Record<string, unknown>> }
    let loadChildren!: (children: Array<Record<string, unknown>>) => void
    const lazyLoad = vi.fn((_row: Record<string, unknown>, load: typeof loadChildren) => {
      loadChildren = load
    })
    ...
    expect(toggleAt(container, 0)).not.toBeNull()
    await fireEvent.click(toggleAt(container, 0)!)
    expect(lazyLoad).toHaveBeenCalledTimes(1)

    loadChildren([])
    await waitFor(() => expect(toggleAt(container, 0)).toBeNull())
    expect(visibleNames(container)).toEqual(['Lazy empty'])
    expect(lazyLoad).toHaveBeenCalledTimes(1)
  })

  it('lazyLoad clears loading on a synchronous error so expansion can retry', async () => {
    const lazyLoad = vi.fn(() => {
      throw new Error('offline')
    })
    const { container } = render(IrisTable, {
      props: { columns: treeCols, data: [{ id: 1, name: 'Retry' }], rowKey: 'id', lazyLoad },
    })
    await fireEvent.click(toggleAt(container, 0)!)
    await waitFor(() =>
      expect(toggleAt(container, 0)!.getAttribute('data-iris-tree-loading')).toBeNull(),
    )
    await fireEvent.click(toggleAt(container, 0)!)
