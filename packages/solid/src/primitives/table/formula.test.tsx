...
describe('@iris-ui-kit/solid IrisTable formula columns (batch EL, iris 独有)', () => {
  ...
  it('text filtering matches the computed value', () => {
    const { container } = render(() => (
      <IrisTable columns={columns} data={rows} rowKey="id" filters={{ total: '30' }} />
    ))
  ...
  it('checked filter sets match the computed value', () => {
    const { container } = render(() => (
      <IrisTable columns={columns} data={rows} rowKey="id" filterValues={{ total: ['100'] }} />
    ))
  ...
  it('summary row aggregates the computed value through the choke point', () => {
    ...
    expect(summary!.querySelector('[data-iris-table-cell="total"]')?.textContent).toBe('150')
  })
  ...
  it('CSV export materializes computed values; originals untouched', () => {
    ...
    render(() => <IrisTable columns={columns} data={rows} rowKey="id" tableRef={tableRef} />)
  ...
  it('editable + formula is display-only: no dblclick editor, no data-editable attr', () => {
    const { container } = render(() => (
      <IrisTable
        columns={[
          { key: 'name', title: 'Name' },
    ...
    const totalCell = container.querySelector('[data-iris-table-cell="total"]')!
    expect(totalCell.getAttribute('data-editable')).toBeNull()
    fireEvent.dblClick(totalCell)
    expect(container.querySelector('[data-iris-table-editor]')).toBeNull()
  })

  it('click-trigger editing also skips formula columns', () => {
    const { container } = render(() => (
      <IrisTable
        columns={[
    ...
    fireEvent.click(container.querySelector('[data-iris-table-cell="total"]')!)
    expect(container.querySelector('[data-iris-table-editor]')).toBeNull()
    const nameCell = container.querySelector('[data-iris-table-cell="name"]')!
    expect(nameCell.getAttribute('data-editable')).toBe('')
  })

  it('row mode opens editors for non-formula editable columns only', () => {
    const { container } = render(() => (
      <IrisTable
        columns={[
          { key: 'name', title: 'Name', editable: true },
    ...
    expect(totalCell.getAttribute('data-editable')).toBeNull()
    expect(
      container.querySelector('[data-iris-table-cell="name"] [data-iris-table-editor]'),
    ).not.toBeNull()
  })
  ...
  it('clipboard TSV carries the computed value (Ctrl+C shadow rows)', async () => {
    ...
    const { container } = render(() => (
      <IrisTable columns={columns} data={rows} rowKey="id" cellRange clipConfig={{ copy: true }} />
    ))
    ...
    fireEvent.keyDown(container.querySelector('[data-iris-table]')!, {
      key: 'c',
      ctrlKey: true,
    })
