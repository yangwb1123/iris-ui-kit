  ...
  >
    ...
    {#each buildMonthMatrix(currentDate(), 0).flat() as date (formatLocalISO(date))}
      ...
      <div
        ...
        style="min-height:64px;padding:4px;border:1px solid var(--iris-color-border,#e5e7eb);border-radius:4px;cursor:{isCurrentMonth
          ? 'pointer'
        ...
        {#each dayEvents as event (event.id)}
          <span
            ...
            style="font-size:0.7em;background:{event.color
              ...
              ? '#fff'
              : 'var(--iris-color-primary,#6366f1)'};border-radius:3px;padding:1px 4px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;cursor:pointer;display:block"
            onclick={(e) => {
              e.stopPropagation()
