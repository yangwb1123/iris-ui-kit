...
test('all package.json files are valid JSON', () => {
  ...
  for (const dir of pkgDirs) {
    ...
    try {
      ...
      assert.ok(pkg.name.startsWith('@iris-ui-kit/'), `${dir} should be @iris-ui-kit/*`)
    } catch (err) {
...
test('all packages reference @iris-ui-kit/core consistently', () => {
  const pkgDirs = ['react', 'vue', 'solid', 'svelte']
  for (const dir of pkgDirs) {
    const pkgPath = resolve(ROOT, `packages/${dir}/package.json`)
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'))
    const coreDep = pkg.dependencies?.['@iris-ui-kit/core'] || pkg.peerDependencies?.['@iris-ui-kit/core']
    assert.ok(coreDep, `${dir} should depend on @iris-ui-kit/core`)
    // Version should be a workspace protocol
    assert.ok(coreDep.startsWith('workspace:'), `${dir} @iris-ui-kit/core should use workspace: protocol, got "${coreDep}"`)
  }
})
