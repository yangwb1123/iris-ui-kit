...
The genuine remaining work is now either (a) the **Combobox/Autocomplete** NEW component (ROADMAP P1 — the one real expansion gap; LARGE, decompose into core controller + 4 adapters), or (b) Blocker-gated (Tauri toolchain, npm-publish decision). Next iteration could start the Combobox core controller (data-view filtering already exists in core to reuse) as a ≤5-file step, OR await a user direction. Avoid marginal/cosmetic churn.

**Iteration count since last STATE refresh:** 4 (iters 5–8).
