...
the same `@iris-ui-kit/core` engines and are held at parity automatically by
...
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| R1  | Framework-agnostic **window manager** — open/close, focus + z-order, minimize, maximize/restore, move/resize (clamped), edge/quadrant snap                        | ✅     | `@iris-ui-kit/core/window` `createWindowManager` (+ pure `snapRect`/`clampRect`/`cascadeRect`)                                                                                         | `packages/core/src/window.test.ts` (17 tests); full gate                                      |
...
| R3  | **Real Iris components** live inside managed windows                                                                                                              | ✅     | `@iris-ui-kit/{react,vue,solid,svelte}` (IrisTable/Button/Badge/Input…) in app views                                                                                                   | typecheck + build                                                                             |
| R4  | **App aggregation** — App Store of installable web apps; config mounted on a user profile (local or cloud)                                                        | ✅     | `@iris-ui-kit/core/profile` `createUserProfile` (pluggable storage) + App Store + app kinds + add-your-own-URL                                                                         | profile tests; parity (App Store); build                                                      |
...
| R11 | **Window-session persistence** — open windows (geometry/state/stack/focus) survive reload                                                                         | ✅     | `@iris-ui-kit/core/window` `serializeSession`/`restoreSession` + per-shell wiring to the profile `session` pref                                                                        | `window.test.ts`; parity (`serializeSession`)                                                 |
| R12 | **Cross-framework parity** — React + Vue + Solid + Svelte on the SAME core                                                                                        | ✅     | the above realized in all four shells (one core, four thin shells)                                                                                                                     | `pnpm check:desktop-parity` (20 apps + 3 skins + 23 features × 4); full monorepo gate 161/161 |
...
| R14 | **Notifications** — apps post toasts (the `notifications` permission, now honored); auto-dismiss + tone; App Store posts on install/uninstall                     | ✅     | `@iris-ui-kit/core/notifications` `createNotificationCenter` + per-shell Toasts + App Store posting                                                                                    | `packages/core/src/notifications.test.ts`; parity (`createNotificationCenter`)                |
| R15 | **Clipboard manager** — clipboard history (the `clipboard` permission, now honored): re-copy (writes the system clipboard), pin, clear                            | ✅     | `@iris-ui-kit/core/clipboard-history` `createClipboardHistory` + a Clipboard app in all 4 shells                                                                                       | `packages/core/src/clipboard-history.test.ts`; parity (`createClipboardHistory`)              |
...
| R17 | **Virtual file system** — a real Files manager (the `storage` permission, now honored): folders + text files, create/edit/rename/delete, persisted to the profile | ✅     | `@iris-ui-kit/core/fs` `createVirtualFs` + a Files manager app in all 4 shells + profile-backed persistence                                                                            | `packages/core/src/fs.test.ts`; parity (`createVirtualFs`)                                    |

## How to verify (all green)

```sh
pnpm turbo run test typecheck lint build   # whole monorepo: 161/161
pnpm check:desktop-parity                  # 4 shells: 20 apps + 3 skins + 23 features
pnpm --filter @iris-ui-kit/core test           # core engines incl. window/commands/profile/notifications
```
...
is met. The set currently stands at R1–R17; no functional requirement remains
...
2. **Bottom-up (completeness audit)** — every capability present in the reference
   ...
   accounted for by R1–R17. **There is no capability in the reference that lacks a
...
with the per-shell feature-marker check (23 markers) + app-catalog + OS-skin checks,
the four shells cannot drift and no capability can exist outside this matrix.

**Therefore "all functional requirements are met" is definitively true:** R1–R17
