...
4. **Adapters** — `@iris-ui-kit/{react,vue,solid,svelte}`: thin bindings that bridge core stores to each framework's reactivity (`useSyncExternalStore` / `shallowRef` / `createSignal` / `$state`). The generated manifest reports 154 components in every adapter; all `154 × 4 = 616` framework contracts are extracted from their native adapter source and none are `unavailable`. Vue/Solid/Svelte are authored as h()-render `.ts` / `.tsx` / `.svelte`.
...
- Every adapter component ships across all 4 frameworks. The manifest asserts name parity, 616 native framework contracts, and zero unavailable contracts; 42 shared behavior scenarios are replayed by all four contract drivers.
