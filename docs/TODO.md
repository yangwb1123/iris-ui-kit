...
- **P1: expand `@iris-ui/core/contracts`** — harness now **13 scenarios** across 12 components (tabs/switch/checkbox/accordion + segmented/toggle-group[single+multiple] + slider + radio + number-input + rating + pagination + stepper), each asserted identical ×4. **Click/key-based + single/multi-select controller paths now covered.** Remaining components are either portal-gated (select/combobox/dropdown-menu/dialog — content mounts outside the container-scoped driver; see iter-26/27 deferral) or have the per-fw-handler-split issue (tree). Contract expansion is effectively complete for what's cleanly testable. **The pagination contract paid for the whole harness** — caught a real Solid uncontrolled-`aria-current` bug (iter-24) per-fw unit tests missed. Thesis validated.
