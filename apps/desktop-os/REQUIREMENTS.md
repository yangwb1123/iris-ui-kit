...
| R17 | **Virtual file system** — a real Files manager (the `storage` permission, now honored): folders + text files, create/edit/rename/delete, persisted to the profile | ✅     | `@iris-ui/core/fs` `createVirtualFs` + a Files manager app in all 4 shells + profile-backed persistence                                                                            | `packages/core/src/fs.test.ts`; parity (`createVirtualFs`)                                    |
