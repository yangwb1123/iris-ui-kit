...
test('config has size budgets for all packages', () => {
  ...
  assert.ok(budgets['@iris-ui-kit/solid'] > 0)
  assert.ok(budgets['@iris-ui-kit/svelte'] > 0)
  assert.ok(budgets['@iris-ui-kit/tokens'] > 0)
  assert.ok(budgets['@iris-ui-kit/theme'] > 0)
})

test('size budget values are reasonable', () => {
  const cfg = getConfig()
  // Core should be smallest
  assert.ok(cfg.size.budgets['@iris-ui-kit/core'] < cfg.size.budgets['@iris-ui-kit/react'],
    'core budget should be smaller than react')
  assert.ok(cfg.size.budgets['@iris-ui-kit/core'] < cfg.size.budgets['@iris-ui-kit/vue'],
    'core budget should be smaller than vue')
  assert.ok(cfg.size.budgets['@iris-ui-kit/manifest'] <= 5,
