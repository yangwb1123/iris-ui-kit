 ...
 * that `useToast()` from any component reaches the same queue — calls don't
 * need a context provider.
 */
...
/** Read current toasts. */
export const getToasts = () => store.getState()
 ...
 *
 * Returns the toast id so the caller can `dismiss(id)` later.
 */
