...
function RowOpsDemo() {
  const [rows, setRows] = createSignal<GridRow[]>([...tableData])
      ...
      toolbar={{
        buttons: [
          ...
          },
          {
            key: 'remove',
            label: '删除末行',
            onClick: () => setRows((prev) => prev.filter((r) => r.id !== 9000)),
          },
        ],
      }}
    />
...
export function VxeGridExamplePage() {
  return (
    <div style={sectionStyle}>
      <section>
        <h2 style={h2Style}>vxe-grid 基础用法（Basic usage）</h2>
        <p style={pStyle}>
      ...
      <section>
        <h2 style={h2Style}>行编辑（Row editing）</h2>
        <p style={pStyle}>
      ...
      <section>
        <h2 style={h2Style}>服务端数据源（Server-side data source）</h2>
        <p style={pStyle}>
