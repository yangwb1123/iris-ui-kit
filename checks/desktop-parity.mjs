...
// Every reference-shell component/app view must map to a declared requirement.
// A new capability therefore fails closed until the acceptance matrix and this
// set are extended together.
...
export async function run() {
  ...
  const referenceCapabilities = [
    ...readdirSync(resolve(ROOT, 'apps', reference, 'src', 'components')),
    ...readdirSync(resolve(ROOT, 'apps', reference, 'src', 'appviews')),
  ]
    .filter(file => /\.tsx?$/.test(file) && !/\.test\./.test(file))
    .map(file => file.replace(/\.tsx?$/, ''))

  for (const capability of referenceCapabilities) {
    if (!MAPPED_REFERENCE_CAPABILITIES.has(capability)) {
      problems.push(
        `${reference}: capability '${capability}' has no mapped requirement — ` +
          'update REQUIREMENTS.md and MAPPED_REFERENCE_CAPABILITIES',
      )
    }
  }
