...
test('size budgets cover all packages', () => {
  const cfg = getConfig()
  const required = ['@iris-ui/core', '@iris-ui/react', '@iris-ui/vue', '@iris-ui/solid', '@iris-ui/svelte']
...
test('coverage targets exist for key packages', () => {
  const cfg = getConfig()
  assert.ok(cfg.coverage.targets['@iris-ui/core'] >= 0)
  assert.ok(cfg.coverage.targets['@iris-ui/react'] >= 0)
})
