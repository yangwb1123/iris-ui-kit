...
- **Submenu hover-open MODEL — DECISION-GATED (needs user, not a clear fix).** React/Vue: debounce submenu open ~100ms AND keep it open when the pointer leaves the trigger (close via Escape/ArrowLeft/outside-click/select — Windows-style). Solid/Svelte: open instantly AND close on pointer-leave. Full "behave identically" parity requires picking ONE model for all four — a genuine UX design choice where neither is clearly correct (the React/Vue stay-open model is divisive; the instant close-on-leave is arguably more intuitive). Left for user direction rather than autonomously imposing one. Touches `IrisMenuSub` ×{the two that don't match the chosen model}.
