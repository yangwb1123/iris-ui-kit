...
<div
  ...
  style="display: flex; flex-direction: {ctx.orientation === 'horizontal'
    ? 'row'
    : 'column'}; border-bottom: {ctx.orientation === 'horizontal'
    ? '1px solid var(--iris-border)'
    : 'none'}; border-inline-end: {ctx.orientation === 'vertical'
    ? '1px solid var(--iris-border)'
    : 'none'}; {style ?? ''}"
>
  {@render children?.()}
