  ...
  >
    ...
    {#each getWeekdayNames(0) as name (name)}
      <div
        ...
        style="text-align:center;font-weight:600;font-size:0.75em;padding:4px 0;color:var(--iris-color-muted,#6b7280)"
      >{name}</div>
    {/each}
    ...
    {#each buildMonthMatrix(currentDate(), 0).flat() as date (formatLocalISO(date))}
      ...
      <div
        ...
        style="min-height:64px;padding:4px;border:1px solid var(--iris-color-border,#e5e7eb);border-radius:4px;cursor:{isCurrentMonth ? 'pointer' : 'default'};opacity:{isCurrentMonth ? '1' : '0.4'};background:transparent;display:flex;flex-direction:column;gap:2px"
        onclick={() => { if (isCurrentMonth) config.onDateClick?.(iso) }}
      >
        <!-- Day number -->
        <span
          data-iris-event-cal-day-num
          style="align-self:flex-start;font-size:0.8em;font-weight:{isToday ? '700' : '400'};background:{isToday ? 'var(--iris-cal-today-bg,#6366f1)' : 'transparent'};color:{isToday ? '#fff' : 'inherit'};border-radius:{isToday ? '50%' : '0'};width:22px;height:22px;display:flex;align-items:center;justify-content:center"
        >{date.getDate()}</span>
        ...
        {#each dayEvents as event (event.id)}
          <span
            ...
            title={event.title}
            style="font-size:0.7em;background:{event.color ? event.color : 'var(--iris-cal-event-bg,rgba(99,102,241,0.15))'};color:{event.color ? '#fff' : 'var(--iris-color-primary,#6366f1)'};border-radius:3px;padding:1px 4px;overflow:hidden;white-space:nowrap;text-overflow:ellipsis;cursor:pointer;display:block"
            onclick={(e) => { e.stopPropagation(); config.onEventClick?.(event) }}
          >{event.title}</span>
        {/each}
      </div>
    {/each}
