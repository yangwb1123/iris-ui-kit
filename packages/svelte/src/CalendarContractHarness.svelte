<script lang="ts">
  ...
  // seeded to `null` (nothing selected initially), and write the emitted date
  // back. `defaultMonth` is fixed to June 2024 so the day cells are deterministic
  // (`data-iris-calendar-day-iso="2024-06-10"` / `"2024-06-20"`), exactly what
  // the shared calendarScenario asserts. Constructing a Date here is fine — this
  // is test infra, not a workflow script.
