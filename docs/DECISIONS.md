...
## ADR-009 — framework contracts are extracted natively, not inferred from a sibling adapter

日期: 2026-07 · 决策: the generated manifest extracts each component contract from its own React, Vue, Solid, or Svelte source and treats any unextractable adapter as a failing `unavailable` gap; it must never substitute another framework's props as a parity shortcut. · 原因: identical export names prove only reachability, while AI/MCP consumers need the actual framework-local prop surface, including type aliases, intersections, imported declarations, and Svelte `$props` shapes. · 影响: the current 154-component inventory produces 616 `source: native` contracts and zero `unavailable` contracts, guarded by manifest tests and generated-reference checks. The older ADR-008 note that described `props.ts` extraction as interface-only best-effort remains historical context but is superseded for the current extractor. · 替代方案: copy the React contract to the other adapters or silently emit empty/unavailable props (rejected — masks real drift).
