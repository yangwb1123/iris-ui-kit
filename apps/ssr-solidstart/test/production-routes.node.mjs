...
async function waitForServer() {
  ...
  throw new Error(`SolidStart production server did not start.\n${serverLog}`)
}
...
test('production file routes render the shared IA and query data', async () => {
  ...
  assert.match(home.html, /data-hydration-demo="solidstart"/)
  assert.match(home.html, /live badge/)
  assert.equal(data.response.status, 200)
  assert.match(data.html, /data-ssr-source="solidstart-query"/)
  assert.match(data.html, /Ada Lovelace/)
  assert.equal(feedback.response.status, 200)
  assert.match(feedback.html, /action="\/_server\?/)
  assert.match(feedback.html, /Send feedback/)
})

test('native server action validates input and renders error/success states', async () => {
  const page = await (await fetch(`${baseUrl}/feedback`)).text()
  const encodedAction = /<form[^>]+action="([^"]+)"/.exec(page)?.[1]
  assert.ok(encodedAction)
  const action = encodedAction.replaceAll('&amp;', '&')
  const headers = {
    accept: 'text/html,application/xhtml+xml',
    origin: baseUrl,
    referer: `${baseUrl}/feedback`,
  }

  const invalid = await fetch(new URL(action, baseUrl), {
    method: 'POST',
  ...
  assert.match(valid.url, /\/feedback\?status=success/)
