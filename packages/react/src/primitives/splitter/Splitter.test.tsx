...
describe('@iris-ui/react IrisSplitter', () => {
  it('renders container + handle + two panes', () => {
    const { container } = render(
      <IrisSplitter start={<div>S</div>} end={<div>E</div>} />,
    )
  ...
  it('handle has role="separator" with correct ARIA values', () => {
    const { container } = render(
      <IrisSplitter value={0.3} start={<div />} end={<div />} />,
    )
  ...
  it('disabled handle has tabindex=-1', () => {
    const { container } = render(
      <IrisSplitter disabled start={<div />} end={<div />} />,
    )
    expect(
      (container.querySelector('[data-iris-splitter-handle]') as HTMLElement).tabIndex,
    ).toBe(-1)
  })

  it('pane flex ratios derive from value', () => {
    const { container } = render(
      <IrisSplitter value={0.25} start={<div />} end={<div />} />,
    )
  ...
  it('defaultValue applies for uncontrolled', () => {
    const { container } = render(
      <IrisSplitter defaultValue={0.7} start={<div />} end={<div />} />,
    )
    const handle = container.querySelector('[data-iris-splitter-handle]')!
