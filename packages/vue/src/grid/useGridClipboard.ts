  ...
  if (!core.hasFeature('clipboard')) {
    core.use(
      createGridClipboardFeature<Row>({
        getRows: () => latest.value.getRows?.() ?? core.invoke<Row[]>('getRows'),
        ...
        resolveValue: (row, column) => {
          const resolve = latest.value.resolveValue
        ...
        setValue: (row, column, value) =>
          latest.value.setValue?.(row, column, value) ?? defaultSetValue(row, column, value),
        isCellEditable: (row, column, rowIndex, columnIndex) =>
          latest.value.isCellEditable?.(row, column, rowIndex, columnIndex) ?? true,
        reconcileRows: (sourceRows, previousRows, rows) =>
          latest.value.reconcileRows?.(sourceRows, previousRows, rows) ?? rows,
        commitOptions: () => {
