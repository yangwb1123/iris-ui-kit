...
- **Residual tree-keyboard parity (small, follow-up):** after iter-18 react/vue/solid trees support Home/End + `aria-level`; **svelte tree still lacks Home/End and `aria-level`**. Close for full 4-fw tree-keyboard parity (1 file: `packages/svelte/src/primitives/tree/IrisTree.svelte` + its test).
