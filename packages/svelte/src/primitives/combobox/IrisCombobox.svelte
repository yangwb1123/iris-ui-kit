<script lang="ts">
  ...
  function handleKeyDown(e: KeyboardEvent): void {
    ...
    if (e.key === 'ArrowDown') {
      e.preventDefault()
      if (!open) { open = true; filtering = false; activeIndex = 0; return }
      ...
      if (open) { e.preventDefault(); activeIndex = list.length - 1 }
    }
  }

  const borderColor = $derived(invalid ? 'var(--iris-danger)' : focused ? 'var(--iris-primary)' : 'var(--iris-border)')
  const activeId = $derived(open && activeIndex >= 0 && filtered()[activeIndex] ? optionId(activeIndex) : undefined)
</script>
...
>
  <input
    ...
    onfocus={() => { if (disabled) return; focused = true; open = true; filtering = false }}
