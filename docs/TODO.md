...
- **P1: expand `@iris-ui/core/contracts`** — harness now **15 scenarios** across 13 components (tabs/switch/checkbox/accordion + segmented/toggle-group[single+multiple] + slider + radio + number-input + rating + pagination + stepper + **Table sort + Table multi-select**), each asserted identical ×4. Covers click/key controls, single+multi-select controller paths, AND the flagship Table's sort + row-selection. **The Table-select contract found+fixed a real a11y gap** (iter-30: vue/solid/svelte rows lacked `aria-selected`). Remaining: portal-gated (select/combobox/dropdown-menu/dialog — deferred, see iter-26/27) or per-fw-handler-split (tree). Further Table contracts possible but lower-value (column-resize, row-expand). **Contract expansion now effectively complete** for the cleanly-testable surface; the harness has paid for itself twice (pagination iter-24, table-select iter-30). **The pagination contract paid for the whole harness** — caught a real Solid uncontrolled-`aria-current` bug (iter-24) per-fw unit tests missed. Thesis validated.
