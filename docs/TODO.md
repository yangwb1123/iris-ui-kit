...
- **P1: expand `@iris-ui/core/contracts`** — harness now covers **12 components** (tabs/switch/checkbox/accordion + segmented/toggle-group/slider + radio + number-input + rating + pagination + iter-26 stepper), each event-script asserted identical ×4 adapters. **Click/key-based candidates are now largely tapped out.** Remaining components are either portal-gated (select/combobox/dropdown-menu/dialog — options/content mount outside the container-scoped driver) or have the per-fw-handler-split issue (tree). **The pagination contract paid for the whole harness** — caught a real Solid uncontrolled-`aria-current` bug (iter-24) per-fw unit tests missed. Thesis validated.
