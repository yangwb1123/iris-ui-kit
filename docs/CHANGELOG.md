...
- **iter 20** — fix(a11y/solid): solid `IrisSelect` gained **Home/End** option navigation (jump to first/last enabled option via core `firstEnabledIndex`/`lastEnabledIndex`), matching react's key set; +2 behavioral tests (End/Home → Enter selects last/first). This is the first fix from a **Select-keyboard parity audit** that surfaced a larger gap (logged in TODO): react has full roving keyboard, but **vue `IrisSelect` has NO keyboard navigation at all**, **svelte lacks arrow navigation** (only Escape + typeahead), and solid lacked Home/End. 126/126 gates green.
