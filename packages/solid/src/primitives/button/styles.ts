/**
 * Singleton stylesheet for the Solid `IrisButton`. Byte-identical CSS to the
 * React and Vue adapters — all three write the same DOM with the same
 * selectors, so the same `<style id>` no-ops if more than one adapter loads.
 */
